function solution(genres, plays) {
    ...
    for(let i = 0; i < genres.length; i ++) {
        ...
        map.get(genre).push({ play: play, index: i})
    }
    for(const songs of map.values()){
        songs.sort((a,b) => b.play - a.play)
    }
    const order = [...map.entries()].sort((a,b) => {
        const orderA = a[1].reduce((acc, cur) => acc + cur.play, 0);
        const orderB = b[1].reduce((acc, cur) => acc + cur.play, 0);
...
//  장르 내에서 많이 재생 된 > 장르 내에서 재생 횟수가 같으면 고유번호 낮은 순
